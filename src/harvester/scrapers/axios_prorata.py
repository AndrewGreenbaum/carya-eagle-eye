"""
Axios Pro Rata RSS Scraper - Monitor Dan Primack's VC newsletter.

STATUS: DISABLED (December 2025)
Axios RSS feeds return 404 - feed deprecated. This scraper returns 0 results.
Deal coverage is provided by alternative sources:
- Brave Search (queries for Axios Pro Rata funding news)
- TechCrunch RSS
- Tech Funding News
"""
...
# DISABLED: Axios RSS feeds return 404 as of December 2025
# Feeds kept as comments for reference only
AXIOS_PRORATA_FEEDS = {
    # "prorata": "https://api.axios.com/feed/pro-rata",
    # "technology": "https://api.axios.com/feed/technology",
    # "deals": "https://api.axios.com/feed/axios-pro-deals",
}
...
FUNDING_KEYWORDS = [
    ...
    "announces", "backs", "invests", "deal", "exclusive", "scoop"
]
...
@dataclass
class AxiosArticle:
    """Single article from Axios RSS feed."""
    title: str
    url: str
    description: str
...
class AxiosProRataScraper:
    """
    Scraper for Axios Pro Rata and deals RSS feeds.

    Monitors Dan Primack's Pro Rata newsletter for VC/PE deals.
    One of the most respected sources for funding news.
    """
    ...
    def __init__(self, feeds: Optional[Dict[str, str]] = None):
        ...
        """
        self.feeds = feeds or AXIOS_PRORATA_FEEDS
    ...
    async def fetch_feed(self, feed_url: str, category: str) -> List[AxiosArticle]:
        ...
        for attempt in range(3):
            try:
                ...
                if feed.bozo:
                    logger.warning(f"Malformed Axios feed {category}: {feed.bozo_exception}")
                    return []

                # Check if feed is valid
                if not feed.entries:
                    logger.debug(f"Axios {category} feed returned no entries (may be deprecated)")
                    return []
                ...
                for entry in feed.entries:
                    ...
                    articles.append(AxiosArticle(
                        title=entry.get('title', ''),
                        url=entry.get('link', ''),
                        description=description,
                        published=pub_date,
                        author=entry.get('author', 'Dan Primack'),
                ...
                if attempt < 2:
                    delay = 2 ** attempt
                    logger.warning(f"HTTP error fetching Axios {category} feed (attempt {attempt + 1}/3), retrying in {delay}s: {e}")
                    await asyncio.sleep(delay)
                else:
                    logger.debug(f"Axios {category} feed unavailable (may be deprecated): {e}")
                    return []
            except Exception as e:
                logger.error(f"Unexpected error fetching Axios {category} feed: {e}", exc_info=True)
                return []

        return []

    async def fetch_all_feeds(self, hours_back: int = 168) -> List[AxiosArticle]:
        ...
        if all_articles:
            logger.info(f"Fetched {len(all_articles)} unique articles from Axios Pro Rata")
        else:
            logger.debug("Axios Pro Rata feeds returned no articles (feeds may be deprecated)")

        return all_articles

    def is_funding_article(self, article: AxiosArticle) -> bool:
        """Check if article is about funding."""
        text = f"{article.title} {article.description}".lower()
        return any(kw in text for kw in FUNDING_KEYWORDS)

    def is_enterprise_ai(self, article: AxiosArticle) -> bool:
    ...
    async def fetch_full_article(self, url: str) -> Optional[str]:
        ...
        try:
            ...
            # Axios article selectors
            for selector in ['.article-body', '.gtm-story-content', 'article', '.story-body', 'main']:
            ...
            logger.error(f"HTTP error fetching Axios article {url}: {e}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error fetching Axios article {url}: {e}", exc_info=True)
            return None
    ...
    async def scrape_all(self, hours_back: int = 168) -> List[NormalizedArticle]:
        ...
        for article in articles:
            ...
            # Build tags
            tags = ['axios_prorata', article.feed_category]
            ...
            normalized.append(NormalizedArticle(
                ...
                author=article.author or "Dan Primack",
                tags=tags,
        ...
        logger.info(f"Scraped {len(normalized)} funding articles from Axios Pro Rata")
        return normalized


# Convenience function
async def run_axios_prorata_scraper(hours_back: int = 168) -> List[NormalizedArticle]:
    """Run Axios Pro Rata scraper and return articles."""
