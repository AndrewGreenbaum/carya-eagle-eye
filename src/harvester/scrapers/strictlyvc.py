"""
StrictlyVC RSS Scraper - Monitor Connie Loizos' VC newsletter.

STATUS: DISABLED (April 2020)
StrictlyVC RSS feed has been dead since April 2020. This scraper returns 0 results.
Connie Loizos now writes for TechCrunch, which is covered by:
...
# DISABLED: StrictlyVC RSS feed dead since April 2020
# Connie Loizos now writes for TechCrunch
STRICTLYVC_FEEDS = {
    # "main": "https://www.strictlyvc.com/feed/",
}
...
FUNDING_KEYWORDS = [
    ...
    "announces", "backs", "invests", "deal", "exclusive"
]
...
@dataclass
class StrictlyVCArticle:
    """Single article from StrictlyVC RSS feed."""
    title: str
    url: str
    description: str
...
class StrictlyVCScraper:
    """
    Scraper for StrictlyVC RSS feed.

    Monitors Connie Loizos' StrictlyVC newsletter for VC deals.
    One of the most respected sources for VC funding news.
    """
    ...
    def __init__(self, feeds: Optional[Dict[str, str]] = None):
        ...
        Args:
            feeds: Dict mapping category to feed URL. If None, uses default feed.
        """
        self.feeds = feeds or STRICTLYVC_FEEDS
    ...
    async def fetch_feed(self, feed_url: str, category: str = "main") -> List[StrictlyVCArticle]:
        ...
        for attempt in range(3):
            try:
                ...
                if feed.bozo:
                    logger.warning(f"Malformed StrictlyVC feed: {feed.bozo_exception}")
                    return []
                ...
                for entry in feed.entries:
                    ...
                    if description:
                        ...
                        description = soup.get_text(strip=True)[:3000]  # StrictlyVC has longer descriptions
                    ...
                    articles.append(StrictlyVCArticle(
                        title=entry.get('title', ''),
                        url=url,
                        description=description,
                        published=pub_date,
                        author=entry.get('author', 'Connie Loizos'),
                ...
                if attempt < 2:
                    delay = 2 ** attempt
                    logger.warning(f"HTTP error fetching StrictlyVC feed (attempt {attempt + 1}/3), retrying in {delay}s: {e}")
                    await asyncio.sleep(delay)
                else:
                    logger.error(f"HTTP error fetching StrictlyVC feed after 3 attempts: {e}")
                    return []
            except Exception as e:
                logger.error(f"Unexpected error fetching StrictlyVC feed: {e}", exc_info=True)
                return []

        return []

    async def fetch_all_feeds(self, hours_back: int = 168) -> List[StrictlyVCArticle]:
        ...
        logger.info(f"Fetched {len(all_articles)} unique articles from StrictlyVC")
        return all_articles

    def is_funding_article(self, article: StrictlyVCArticle) -> bool:
        """Check if article is about funding."""
        text = f"{article.title} {article.description}".lower()
        return any(kw in text for kw in FUNDING_KEYWORDS)

    def is_enterprise_ai(self, article: StrictlyVCArticle) -> bool:
    ...
    async def fetch_full_article(self, url: str) -> Optional[str]:
        ...
        try:
            ...
            # StrictlyVC article selectors
            for selector in ['.entry-content', '.post-content', 'article', '.newsletter-content', 'main']:
            ...
            logger.error(f"HTTP error fetching StrictlyVC article {url}: {e}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error fetching StrictlyVC article {url}: {e}", exc_info=True)
            return None
    ...
    async def scrape_all(self, hours_back: int = 168) -> List[NormalizedArticle]:
        ...
        for article in articles:
            ...
            normalized.append(NormalizedArticle(
                ...
                author=article.author or "Connie Loizos",
                tags=tags,
        ...
        logger.info(f"Scraped {len(normalized)} funding articles from StrictlyVC")
        return normalized


# Convenience function
async def run_strictlyvc_scraper(hours_back: int = 168) -> List[NormalizedArticle]:
    """Run StrictlyVC scraper and return articles."""
